...
class InfluencerViewmodel extends GetxController {
  ...
  List<InfluencerModel> get filteredInfluencers {
    if (searchQuery.isEmpty) return influencers;
    final query = searchQuery.value.toLowerCase();
    return influencers.where((influencer) {
      final name = influencer.name.toLowerCase();
      final id = influencer.id.toString().toLowerCase();
      final email = influencer.email.toLowerCase();
  ...
  Future<void> deleteUser(int uid) async {
    try {
      ...
      final response = await http.delete(
        Uri.parse(
            'https://cactisocial.com/api-clique/public/api/v1/user/delete/$uid'),
        headers: {
  ...
  Future<void> updateUserRole(int uid, String newRole) async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            'https://cactisocial.com/api-clique/public/api/v1/user/update-role'), // ✅ Updated path
        headers: {
          'Accept': 'application/json',
  ...
  void fetchUsers() async {
    try {
      ...
      isLoading(
          false); // Set loading to false after fetching data (whether successful or not)
    }
  }
}
